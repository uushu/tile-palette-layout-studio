    ...
    internal static class OllamaVisionClient
    {
        ...
        internal static bool HasRequiredModel(string responseJson)
        {
            ...
            foreach (ModelInfo model in response.models)
            {
                ...
                if (string.Equals(model.name, Model, StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(model.model, Model, StringComparison.OrdinalIgnoreCase))
                    return true;
            }
        ...
        internal static OllamaGenerateResult ParseGenerateResponse(string responseJson)
        {
            ...
            if (string.IsNullOrWhiteSpace(response.response))
                throw new InvalidOperationException("Response does not contain generated content.");
            ...
            string rawValue)
        {
            if (string.IsNullOrEmpty(json) || json[json.Length - 1] != '}')
                throw new InvalidOperationException("Cannot build Ollama request JSON.");

...
namespace TilePaletteLayoutStudio
{
    ...
    public sealed class TilePaletteProfileGroup
    {
        ...
        public string subgroupName = "main";
        public Vector3Int origin;
        public List<TilePaletteProfileEntry> entries = new List<TilePaletteProfileEntry>();

        public string Id => string.IsNullOrWhiteSpace(subgroupName)
            ? groupName
            : groupName + "/" + subgroupName;
    }

    [CreateAssetMenu(menuName = "Tile Palette/Layout Profile", fileName = "TilePaletteProfile")]
    public sealed class TilePaletteProfile : ScriptableObject
    {
        ...
        public IEnumerable<TilePaletteProfileEntry> IncludedEntries =>
            groups.SelectMany(group => group.entries).Where(entry => entry.included);
        ...
        public void ReplaceGroups(IEnumerable<TilePaletteProfileGroup> newGroups)
        {
            groups = newGroups == null
                ? new List<TilePaletteProfileGroup>()
                : newGroups.ToList();
        }

        public void MarkBuilt(string hash)
        {
            lastBuildHash = hash;
        }
    }
